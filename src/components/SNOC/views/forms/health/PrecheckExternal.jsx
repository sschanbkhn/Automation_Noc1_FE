// pages/Precheck/PrecheckExternal.jsx
// Test Precheck External API — POST 202 job_id → poll GET → kết quả giống PrecheckManual
...
const PrecheckExternal = () => {
  ...
  const pollJob = useCallback(async (id) => {
    try {
      const res = await snocApi.get(`nornirps/precheck/job/${id}/`);
      const d   = res.data;
  ...
  const handleRun = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await snocApi.post("nornirps/precheck/external/", {
        node_names,
  ...
  const expandAll   = () => {
    const all = {};
    results.forEach((r) => { all[r.hostname] = true; });
  ...
  const handleExport = () => {
      ...
      (r.ketquaprecheck || []).forEach((grp) => {
        (grp.congviecchitiet || []).forEach((task) => {
          rows.push({
            ...
            "IP":           r.ip,
            "Platform":     r.platform || "",
            "Nhóm":         grp.tennhom || `Nhóm ${grp.idnhomcongviec}`,
    ...
    saveAs(
      ...
      `precheck_external_${new Date().toISOString().slice(0, 10)}.xlsx`
    );
  };
      ...
      <div className="container-fluid px-3 py-3">
        ...
        <Card className="mb-3 shadow-sm">
          <Card.Header className="fw-semibold py-2" style={{ background: "#f8f9fa" }}>
            🔍 Precheck External API
            <span className="text-muted fw-normal ms-2" style={{ fontSize: "0.8rem" }}>
              (Async — tự detect platform, không cần chọn platform)
            </span>
          </Card.Header>
          <Card.Body>
          ...
          <Card body className="text-center py-5 shadow-sm mb-3">
            ...
            {jobId && !submitting && (
              <div className="mt-1 text-muted" style={{ fontSize: "0.75rem" }}>
                GET <code>{`nornirps/precheck/job/${jobId}/`}</code>
              </div>
            )}
          </Card>
        )}
                  ...
                  {results.map((r) => {
                    ...
                    return (
                      <React.Fragment key={r.hostname}>
                        ...
                        {/* Detail collapse row — giống PrecheckManual */}
...
export default PrecheckExternal;
