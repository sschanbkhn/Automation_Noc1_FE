// src/pages/kpi/KPIExplorerUnified.jsx
...
/** Tách prefix KPI:
 * - Nếu có ký tự phân tách: lấy phần trước ký tự đầu tiên trong ["-", "_", ".", "/", ":"]
 * - Nếu không có: cắt theo CamelCase (số coi như thường, đi kèm chữ trước nó)
 */
function getKpiPrefix(name = "") {
  if (!name) return "";

  // 1) Ưu tiên ký tự phân tách
  const splitters = ["-", "_", ".", "/", ":"];
  ...
  // 2) Fallback CamelCase (số đi cùng cụm trước)
  // Ví dụ: "SgwS4S11xxx" → ["Sgw","S4","S11xxx"]
...
const KPIChartDashboard = () => {
  ...
  // ====== KPI options (lọc theo prefix) + THÊM 'chọn tất cả KPI'
  ...
  // 📌 Ghim tất cả KPI đang chọn cho selectedPlatform
              ...
              <Select
                ...
                onChange={(selected, meta) => {
                  if (!selected) {
                    ...
                    // Nếu chọn "tất cả KPI" → chọn toàn bộ KPI hiện có (theo prefix filter)
      ...
      {viewMode === "all-in-one" && selectedKPIs.length > 0 && (
        <Row className="mb-4">
          <Col md={12}>
            <Card>
              <Card.Body>
                {/* Badge KPI có nút ✕ (giữ để bỏ nhanh) */}
                ...
                {/* Bỏ dòng “Tổng hợp: …” để tiết kiệm diện tích */}
