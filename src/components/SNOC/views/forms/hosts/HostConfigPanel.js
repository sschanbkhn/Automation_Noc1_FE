...
const HostManager = () => {
  ...
  const handleEdit = (host) => {
    ...
    setNewHost({
      ...
      group: isAdmin
        ? groupObj?.id || ""
        : (userClaims?.group_id ?? groupObj?.id ?? ""),
      department: isAdmin
        ? deptObj?.id || ""
        : (userClaims?.department_id ?? deptObj?.id ?? ""),
      groups: host.groups?.join(", ") || "",
      username: host.username === "—" ? "" : host.username,
      password: "",
      port: host.port ?? "22",
                  ...
                  >
                    ...
                    <tbody>
                      {paginatedItems.length > 0 ? (
                        ...
                        <tr>
                          <td colSpan="11">Không tìm thấy thiết bị nào.</td>
                        </tr>
                      )}
                    </tbody>
                  </Table>
