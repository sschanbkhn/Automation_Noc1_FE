...
const getStatusBadge = (item) => {
  ...
  if (isPartial)     return <Badge bg="warning" text="dark">PARTIAL SUCCESS</Badge>;
  if (item.status?.toLowerCase() === "success") return <Badge bg="success">SUCCESS</Badge>;
...
const DhttHcSimpleHistory = () => {
  const dispatch = useDispatch();
  const { history: items = [], historyLoading: loading = false, historyCount: count = 0 } =
  ...
  const paramsRef = useRef({ host: "", hours: "24", start: "", end: "" });

  const doFetch = (page = 1, params = paramsRef.current) => {
    dispatch(fetchDhttHcSimpleHistory({
      host:      params.host,
  ...
  const processedItems = [...items]
    .filter(item => {
      ...
      const isPartial = item.status?.toLowerCase() === "success" &&
        item.response_body?.node_khong_ton_tai?.length > 0;
  ...
  const exportToExcel = () => {
    const data = processedItems.map((item, i) => ({
      ...
      "Node không tồn tại": (item.response_body?.node_khong_ton_tai || []).join(", ") || "—",
    }));
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "HCSimple_History");
    const buf = XLSX.write(wb, { bookType: "xlsx", type: "array" });
    saveAs(
      new Blob([buf], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }),
      `HCSimple_History_${new Date().toISOString().slice(0, 10)}.xlsx`
    );
  };
    ...
    <>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} size="lg" centered>
        <Modal.Header closeButton>
          <Modal.Title>Chi tiết HC Simple — ĐHTT Response</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedDetail && (
            <div>
              ...
              <Row className="mb-3">
                ...
                <Col sm={8}>
                  ...
                  <span className="ms-2 text-muted small">HTTP {selectedDetail.status_code}</span>
                </Col>
              </Row>
              <hr />
              ...
              <Table bordered size="sm" className="mt-2">
                <tbody>
                  <tr>
                    <td className="bg-light" style={{ width: "45%" }}>Node không tồn tại trên ĐHTT:</td>
                    <td>
              ...
              <h6 className="fw-bold text-primary mt-3">Payload gửi đi (id_thamso=112):</h6>
              <pre style={{
                background: "#f8f9fa", padding: "12px", borderRadius: "5px",
                maxHeight: "200px", overflow: "auto", fontSize: "11px",
              }}>
                {JSON.stringify(
                  (selectedDetail.payload_sent || []).map(n => ({
                    hostname: n.hostname,
                    kq_112: (n.listKqbd || []).find(k => k.id_thamso === 112),
                  })),
                  null, 2
                )}
              </pre>
              <h6 className="fw-bold text-primary mt-3">Response JSON đầy đủ:</h6>
              <pre style={{
                background: "#f8f9fa", padding: "12px", borderRadius: "5px",
                maxHeight: "200px", overflow: "auto", fontSize: "11px",
              }}>
          ...
          <Card>
            <Card.Header>
              <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
                <Card.Title as="h5" className="mb-0">🛡️ Lịch sử HC Simple — ĐHTT Sync</Card.Title>
                <Button variant="outline-success" onClick={exportToExcel}>📥 Xuất Excel</Button>
              </div>
              ...
              <Form onSubmit={handleSearch} className="mt-3">
                <Row className="g-2 align-items-end">
                  ...
                  <Col md={2}>
                    ...
                    <Form.Select value={timeMode} onChange={e => {
                    ...
                    <Col md={2}>
                      ...
                      <Form.Select value={hours} onChange={e => setHours(e.target.value)}>
                        {[1,3,6,12,24,48,72,168,336,720].map(h => (
                          <option key={h} value={h}>{h < 24 ? `${h} giờ qua` : `${h/24} ngày qua`}</option>
                        ))}
                      </Form.Select>
                    </Col>
                  )}
                ...
                <>
                  <div className="d-flex justify-content-between align-items-center px-3 pt-3 pb-2">
                    ...
                    <InputGroup style={{ width: 140 }}>
                      ...
                      <Form.Select size="sm" value={pageSize}
                    ...
                    <tbody>
                      {processedItems.length === 0 ? (
                        ...
                        <tr key={item.id || index}>
                          ...
                          <td>{item.platform}</td>
                          <td style={{ fontSize: "0.82rem" }}>{parseNodes(item.nodes_attempted)}</td>
...
export default DhttHcSimpleHistory;
