...
import { fetchDhttHcSimpleHistory } from "../../../redux/Healthcheck/dhttHcSimpleSlice";
  ...
  return (
    <div
      ...
      style={{
        ...
        fontSize: "0.78rem", margin: "2px", transition: "transform 0.15s",
      }}
...
const PlatformCard = ({ platform, schedules, onScheduleClick }) => {
  ...
  return (
    <Card style={{ border: `2px solid ${cfg.bg}`, borderRadius: 10, height: "100%" }}>
      <Card.Header style={{ background: cfg.bg + "18", borderBottom: `1px solid ${cfg.bg}44`, padding: "8px 12px" }}>
        <div className="d-flex align-items-center justify-content-between">
          <span className="fw-bold" style={{ fontSize: "0.88rem" }}><code>{platform}</code></span>
          <span style={{ fontSize: "0.72rem", fontWeight: "bold", color: cfg.bg, background: cfg.bg + "22", padding: "1px 8px", borderRadius: 10 }}>
            {cfg.icon} {cfg.label}
          </span>
        </div>
      ...
      <Card.Body style={{ padding: "8px 10px" }}>
        <div className="d-flex flex-wrap">
          {schedules.map(s => (
            <ScheduleChip key={s.id} schedule={s} onClick={onScheduleClick} flash={false} />
          ))}
        </div>
      </Card.Body>
    </Card>
  ...
  return (
    <ResponsiveContainer width="100%" height={130}>
      <BarChart data={data} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
        ...
        <YAxis domain={[0, 1]} ticks={[0, 1]} tickFormatter={v => v === 1 ? "OK" : "Fail"} tick={{ fontSize: 10 }} />
        <Tooltip formatter={(v, _, props) => [props.payload.code, props.payload.label]} labelFormatter={l => `Lúc: ${l}`} />
        <Bar dataKey="status" radius={[3, 3, 0, 0]}>
          {data.map((d, i) => <Cell key={i} fill={d.status === 1 ? "#198754" : "#dc3545"} />)}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
  ...
  return (
    <Modal show onHide={onHide} centered size="md">
      <Modal.Header closeButton>
        <Modal.Title style={{ fontSize: "1rem" }}>{s.icon} {schedule.name}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Table size="sm" bordered>
          <tbody>
            <tr><td className="fw-bold bg-light" style={{ width: "35%" }}>Platform</td><td><code>{schedule.platform}</code></td></tr>
            <tr><td className="fw-bold bg-light">Nodes</td><td style={{ fontSize: "0.82rem" }}>{(schedule.node_names || []).join(", ")}</td></tr>
            <tr><td className="fw-bold bg-light">Cron</td><td><code>{schedule.cron}</code></td></tr>
            <tr>
              <td className="fw-bold bg-light">Trạng thái</td>
              <td><span style={{ color: s.bg, fontWeight: "bold" }}>{s.icon} {s.label}</span></td>
            </tr>
            <tr><td className="fw-bold bg-light">Lần chạy cuối</td><td>{fmtDate(schedule.last_run_at)}</td></tr>
            <tr><td className="fw-bold bg-light">Tóm tắt</td><td style={{ fontSize: "0.82rem", whiteSpace: "pre-line" }}>{schedule.last_result_summary || "—"}</td></tr>
          </tbody>
        </Table>
      </Modal.Body>
      <Modal.Footer>
        <Button variant="secondary" onClick={onHide}>Đóng</Button>
        <Button variant="outline-info" size="sm" onClick={() => { onHide(); navigate("/hc-simple/history"); }}>
          📋 Xem History
        </Button>
        <Button variant="primary" size="sm" onClick={() => { onHide(); onManual(schedule); }}>
          ▶️ Chạy Manual
        </Button>
      </Modal.Footer>
    </Modal>
...
const DhttHcSimpleDashboard = () => {
  ...
  const { history: historyItems = [], historyLoading: histLoading } =
  ...
  useEffect(() => {
    ...
    dispatch(fetchDhttHcSimpleHistory({ hours: 24, page_size: 20 }));
  }, [dispatch]);
  ...
  const handleManual = (schedule) => {
    navigate("/hc-simple/manual", { state: { platform: schedule.platform, node_names: schedule.node_names } });
  };
    ...
    <>
      ...
      <div className="px-2 py-2">
        <div className="d-flex align-items-center justify-content-between mb-3">
          <div>
            <h4 className="mb-0 fw-bold">🛡️ Bảo Dưỡng HC Simple — Dashboard</h4>
            <small className="text-muted">Real-time · Auto-refresh {autoRefresh ? "ON" : "OFF"}</small>
          </div>
          <div className="d-flex gap-2">
            <Button variant={autoRefresh ? "success" : "outline-secondary"} size="sm"
            ...
            <Button variant="primary" size="sm" onClick={() => navigate("/hc-simple/manual")}>
              ▶️ Manual
            </Button>
            <Button variant="outline-secondary" size="sm" onClick={() => navigate("/hc-simple/schedule")}>
              📅 Lịch
            </Button>
            <Button variant="outline-info" size="sm" onClick={() => navigate("/hc-simple/history")}>
              📋 History
            </Button>
          </div>
        </div>
          ...
          {[
            ...
            <Col key={c.label} xs={6} md={2}>
              <div style={{
                ...
                <div>
                  <div style={{ fontSize: "1.6rem", fontWeight: "bold", color: c.bg, lineHeight: 1 }}>
                    {c.value}
                  </div>
                  <div style={{ fontSize: "0.75rem", color: "#555" }}>{c.label}</div>
                </div>
              ...
              {Object.entries(byPlatform).length === 0 ? (
                <Col>
                  <Card body className="text-center text-muted">
                    Chưa có lịch HC Simple nào.{" "}
                    <Button variant="link" size="sm" onClick={() => navigate("/hc-simple/schedule")}>
                      Tạo lịch ngay
                    </Button>
                  </Card>
                </Col>
                ...
                <Col key={platform} md={6} xl={4}>
                  <PlatformCard platform={platform} schedules={sched} onScheduleClick={setSelectedSchedule} />
                </Col>
              ))}
            </Row>
          </Col>
          ...
          <Col md={4}>
            <Row className="g-3">
              <Col md={12}>
                <Card>
                  <Card.Header>
                    <Card.Title as="h6" className="mb-0">📊 Kết quả sync ĐHTT (24h)</Card.Title>
                  </Card.Header>
              ...
              <Col md={12}>
                <Card>
                  <Card.Header className="d-flex justify-content-between align-items-center">
                    <Card.Title as="h6" className="mb-0">📡 Đồng bộ gần nhất</Card.Title>
                    <Button variant="link" size="sm" className="p-0"
                      onClick={() => navigate("/hc-simple/history")}>
                      Xem tất cả →
                    </Button>
                  </Card.Header>
                    ...
                    <Table size="sm" className="align-middle mb-0">
                      ...
                      <tbody>
                        {historyItems.slice(0, 6).map((h, i) => (
                          <tr key={h.id || i}>
                            <td style={{ fontSize: "0.78rem" }}>
                              ...
                              <div className="text-muted" style={{ fontSize: "0.7rem" }}>{h.platform}</div>
                            </td>
                            <td style={{ textAlign: "center" }}>
                              <Badge bg={h.status_code === 200 ? "success" : "danger"} style={{ fontSize: "0.7rem" }}>
                                {h.status_code || "—"}
                              </Badge>
                            </td>
                        ...
                        {historyItems.length === 0 && (
                          <tr>
                            <td colSpan={3} className="text-center text-muted py-3">Không có dữ liệu</td>
                          </tr>
                        )}
                      </tbody>
                    </Table>
      ...
      <style>{`
        .schedule-flash { animation: flash-border 0.8s ease; }
...
export default DhttHcSimpleDashboard;
