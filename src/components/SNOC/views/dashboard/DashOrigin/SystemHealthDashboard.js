...
// Hỗ trợ cả state đã/hoặc chưa tách last_updated ra khỏi systemStatus:
const selectSystemLastUpdated = (s) =>
  s.pscore?.last_updated ??
  s.pscore?.systemStatus?.last_updated ??
  null;
...
const GroupNokChart = ({
  ...
  // Tooltip tùy biến: liệt kê host NOK của giờ đang hover
  const CustomTooltip = ({ active, payload, label }) => {
    if (!active || !payload || !payload.length) return null;
    const arr = hourIndex.get(label) || [];
    const uniqHosts = Array.from(new Set(arr.map((x) => x.host)));
...
const SystemHealth = () => {
  ...
  // ✅ Danh sách group: ưu tiên từ schema; nếu schema rỗng fallback theo systemStatus
  const groupNames = useMemo(() => {
    const ks = Object.keys(platformSchema || {});
    if (ks.length) return ks;
    return Object.keys(systemStatus || {}); // slice đã nên tách last_updated ra; nếu chưa tách vẫn ổn vì select đã guard
  }, [platformSchema, systemStatus]);

  // ✅ Map group -> union platforms (từ schema); nếu schema rỗng => {}
  const groupPlatformsMap = useMemo(() => {
    const map = {};
    Object.entries(platformSchema || {}).forEach(([groupName, subsystems]) => {
      const set = new Set();
      ...
      <div className={styles.container}>
        <Row>
          <Col md={12}>
            ...
            {/* Empty-state nếu không có group và không loading */}
            {!loading && groupNames.length === 0 && (
              <Card className="mb-3">
                <Card.Body className="text-muted">
                  Không có dữ liệu hiển thị. Kiểm tra API
                  <code> /systemhealth/schema/</code> và <code>/systemhealth/</code>.
                </Card.Body>
              </Card>
            )}
            ...
            <Row>
              {groupNames.map((groupName) => {
                ...
                // Lấy subsystems: từ schema nếu có, nếu không fallback theo data thực
                                ...
                                <div className="d-flex flex-wrap gap-3 mt-2">
                                  {Object.keys(subsystems).map(
                                    (subsystemLabel) => {
                                      ...
                                      // Lấy danh sách platform nếu có schema
                                      const platforms =
