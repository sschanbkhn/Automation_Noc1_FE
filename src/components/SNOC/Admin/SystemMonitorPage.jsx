...
const KpiCard = ({ icon, value, label, color }) => (
  ...
  >
    <Card.Body className="py-2 px-1">
      ...
      <div style={{ fontSize: "0.68rem", color: "#6c757d", lineHeight: 1.2 }}>
        {label}
      </div>
    </Card.Body>
  </Card>
      ...
      <Container fluid>
        ...
        <Navbar.Collapse id="nav-col" className="w-100">
          ...
          >
            ...
            {isAdmin && (
              <NavDropdown
                title={
                  <span
                    style={ddTitleStyle(["/healthcheck/monitor", "/admin"])}
                  >
                    ⚙️ Monitor
                  </span>
                }
          ...
          <Nav className="ms-auto align-items-center" style={{ gap: 10 }}>
            ...
            <button
              className="btn btn-outline-light align-items-center d-flex"
...
const MonitorDashboard = () => {
  ...
  const { isConnected, hasEverConnected, sendRefresh } = useMonitorWebSocket(
    onSnapshot,
    onJobDone,
  );
  ...
  return (
    ...
    >
      ...
      <Row className="g-2 mb-3">
        <Col xs={6} md={2}>
          <KpiCard
            ...
            color={locks.length > 0 ? "#dc3545" : "#198754"}
          />
        </Col>
        <Col xs={6} md={2}>
          <KpiCard
            icon="⏳"
            value={qcount.pending || 0}
            label="Pending"
            color="#6c757d"
          />
        </Col>
                    ...
                    {locks.map((lk) => {
                      ...
                      return (
                        <tr key={lk.key} style={{ fontSize: "0.8rem" }}>
                          ...
                          <td className="text-center">
                            <OverlayTrigger
                              overlay={<Tooltip>Force release lock</Tooltip>}
                            >
                              <Button
                                size="sm"
                ...
                <Table size="sm" className="mb-0 align-middle">
                  ...
                  <tbody>
                    {filteredJobs.map((job) => {
                      const mc = MODULE_CFG[job.module] || {
                        color: "#6c757d",
                        icon: "?",
                      };
                      return (
      ...
      <Modal
        show={showRelAll}
        onHide={() => setShowRelAll(false)}
        size="sm"
        centered
      >
        <Modal.Header closeButton>
          <Modal.Title style={{ fontSize: "1rem" }}>
            Xác nhận release all
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
      ...
      >
        ...
        <Modal.Body style={{ fontSize: "0.82rem" }}>
          {jobDetail &&
            (() => {
              const mc = MODULE_CFG[jobDetail.module] || {
                color: "#6c757d",
                icon: "?",
              };
              return (
                <>
                  ...
                  {jobDetail.result?.summary && (
                    <div className="d-flex gap-4 mb-2">
                      {Object.entries(jobDetail.result.summary).map(
                        ([k, v]) => (
