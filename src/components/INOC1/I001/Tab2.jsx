...
import { mockTab2ASNData, DEVICES_LIST } from './mockDataTab2';
        ...
        {data.map((row) => (
          <React.Fragment key={row.id}>
            <tr className="asn-row" onClick={() => onRowClick(row)}>
              ...
              <td className="text-center"><span className="expand-icon">{expandedASNId === row.id ? '▼' : '▶'}</span></td>
            </tr>
                        ...
                        <div key={idx} className="prefix-detail-item">
                          <div className="prefix-info">
                            <div className="prefix-name">
                              ...
                              {prefix.isCountered && <span className="countered-badge">Đã Counter</span>}
                            </div>
                          </div>
...
const AddCounterModal = ({ asnList, onApply, onCancel }) => {
  ...
  const isFormValid = inputASN.trim() !== '' && inputASName.trim() !== '' && selectedPrefixes.length > 0 && selectedDevices.length > 0;
        ...
        <div className="add-counter-section">
          ...
          <div className="asn-input-group">
            <div className="input-field">
              ...
              <input id="asn-input" type="text" placeholder="e.g., AS64512" value={inputASN} onChange={(e) => handleASNChange(e.target.value)} className="form-input" />
            </div>
            <div className="input-field">
              <label htmlFor="asname-input">AS Name</label>
              <input id="asname-input" type="text" placeholder="e.g., TECHNET-AS" value={inputASName} onChange={(e) => setInputASName(e.target.value)} className="form-input" disabled={!!foundASN} />
            </div>
          </div>
        </div>
              ...
              {availablePrefixes.map((prefix) => {
                ...
                return (
                  <label key={prefix.prefix} className={`prefix-selection-item ${isDisabled ? 'disabled' : ''}`}>
                    <input type="checkbox" checked={selectedPrefixes.includes(prefix.prefix)} onChange={(e) => !isDisabled && handlePrefixChange(prefix.prefix, e.target.checked)} disabled={isDisabled} />
        ...
        <div className="add-counter-section">
          ...
          <div className="select-all-control">
            <label className="checkbox-label">
              <input type="checkbox" checked={isAllDevicesSelected} onChange={(e) => handleSelectAllDevices(e.target.checked)} />
          ...
          <div className="device-grid-tab2">
            {DEVICES_LIST.map((device) => (
              <label key={device} className="device-checkbox-item">
                <input type="checkbox" checked={selectedDevices.includes(device)} onChange={(e) => handleDeviceChange(device, e.target.checked)} />
...
const ConfirmApplyModal = ({ asn, asName, prefixes, devices, onApply, onCancel }) => (
  <div className="modal-overlay" onClick={onCancel}>
  ...
  return (
    <div className="tab2-container">
      <div className="tab2-header">
        ...
        <div className="header-right">
          <div className="date-picker-group">
            ...
            <input id="tab2-date-picker" type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="date-picker-input" />
          </div>
      ...
      {modalState === 'addCounter' && (
        <AddCounterModal asnList={asnData} onApply={handleAddCounterApply} onCancel={handleAddCounterCancel} />
      )}
      {modalState === 'confirm' && addCounterData && (
        <ConfirmApplyModal asn={addCounterData.asn} asName={addCounterData.asName} prefixes={addCounterData.selectedPrefixes} devices={addCounterData.selectedDevices} onApply={handleConfirmApply} onCancel={handleConfirmCancel} />
      )}
