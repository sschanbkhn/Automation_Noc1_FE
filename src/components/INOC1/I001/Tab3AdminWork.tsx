          ...
          {data.length > 0 ? (
            data.map((item) => (
              <tr key={item.id} className="ipt-row">
                ...
                <td className="capacity-cell"><span className="capacity-badge">{item.capacity}</span></td>
            ...
            <tr>
              <td colSpan={8} className="empty-state">
                <div className="empty-message"><span>Không có dữ liệu IPT</span></div>
              </td>
            </tr>
          )}
        </tbody>
    ...
    <div className="modal-overlay" onClick={onCancel}>
      <div className="modal-content add-ipt-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-footer">
          ...
          <button className="btn btn-primary" onClick={handleApply} disabled={!isFormValid}>Add IPT</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="modal-overlay" onClick={onCancel}>
      <div className="modal-content set-trigger-alarm-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-footer">
          <button className="btn btn-secondary" onClick={onCancel}>Cancel</button>
          <button className="btn btn-trigger-alarm-action" onClick={handleApply}>Set Trigger</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="modal-overlay" onClick={onCancel}>
      <div className="modal-content set-rollback-time-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="rollback-info">
          ...
          <span className="info-text">Hệ thống sẽ tự động thực hiện rollback toàn bộ cấu hình vào thời gian này mỗi ngày</span>
        </div>

        <div className="modal-footer">
          <button className="btn btn-secondary" onClick={onCancel}>Cancel</button>
          <button className="btn btn-rollback-time-action" onClick={handleApply}>Set Time</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="tab3-container">
      <div className="tab3-header">
        ...
        <div className="header-right">
          <button className="btn btn-add-ipt" onClick={() => setModalState('addIPT')}>
            + Add IPT
          </button>
          <button className="btn btn-trigger-alarm" onClick={() => setModalState('triggerAlarm')}>
            ⚠ Set Trigger Alarm
          </button>
          <button className="btn btn-rollback-time" onClick={() => setModalState('rollbackTime')}>
