...
// import { R007TemplateService } from "./R007TemplateService";
import { R007TemplateService, TemplateListResponse } from "./R007TemplateService";
...
// ============================================================================
  ...
  // ← THÊM FUNCTION loadTemplates Ở ĐÂY (TRƯỚC useEffect)
    const loadTemplates = async () => {
      try {
        if (!state.commissionType) {
          setState((prev) => ({ ...prev, error: "Commission type not selected" }));
          return;
        }
  
        console.log(`🔄 Loading templates for ${state.commissionType}...`);
  
        let response: TemplateListResponse;
  
        if (state.commissionType === "AutoPnP") {
          response = await R007TemplateService.getPnPTemplateList();
        } else {
          response = await R007TemplateService.getOnSiteTemplateList();
        }
        ...
        if (response.success) {
          ...
          console.log(`✅ Loaded ${response.count} templates for ${state.commissionType}`);
        } else {
          setState((prev) => ({
            ...prev,
        ...
        setState((prev) => ({
          ...prev,
          error: "Error loading templates from server. Please check your connection.",
        }));
      }
    };
  //========================================================================
