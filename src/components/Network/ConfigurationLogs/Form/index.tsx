...
import configurationLogsFormInputJson from './FormInput.json';
interface Props {
  Id: string,
...
const ConfigurationLogsForm = (props: Props) => {  
  const [state, dispatch] = useReducer(Reducer, InitState)
  useEffect(() => {
    Actions.GetItem(props.Id, dispatch);
  }, [props.Id])
  let configurationLogsFormInput:any = configurationLogsFormInputJson;
  ...
  return(
    <>
      ...
      <CtrlDynamicForm options={props.Options} ref={refDynamicForm} initValues={state.DataItem} formDefs={configurationLogsFormInput} actionEvents={ActionEvents} />
    </>
  )
}
...
export default connect(mapState, mapDispatchToProps)(ConfigurationLogsForm);