...
import cableManagementFormInputJson from './FormInput.json';
interface Props {
  Id: string,
...
const CableManagementForm = (props: Props) => {  
  const [state, dispatch] = useReducer(Reducer, InitState)
  useEffect(() => {
    Actions.GetItem(props.Id, dispatch);
  }, [props.Id])
  let cableManagementFormInput:any = cableManagementFormInputJson;
  ...
  const ActionEvents = {
    onClickSave: async () => {
      ...
      {        
        ...
        res = await Actions.CheckDuplicateAttributes(stateValues.Id, stateValues.CableCode, dispatch);
        if(res.Data) 
        {
  ...
  return(
    <>
      ...
      <CtrlDynamicForm options={props.Options} ref={refDynamicForm} initValues={state.DataItem} formDefs={cableManagementFormInput} actionEvents={ActionEvents} />
    </>
  )
}
...
export default connect(mapState, mapDispatchToProps)(CableManagementForm);