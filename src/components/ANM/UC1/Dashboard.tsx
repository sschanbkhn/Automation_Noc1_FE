...
const Dashboard: React.FC<DashboardProps> = ({ goToTab , setFilters}) => {
  ...
  const handleClickSlice = (label: string) => {
    if (label === 'Fail') {
      if (goToTab && setFilters) {
        setFilters({ province, olt, vendor }); // push filters to UC2
        goToTab('uc2'); // switch tab layout
      } else {
  ...
  const handleBarClick = (provinceData: any) => {
    if (goToTab && setFilters) {
      setFilters({ province: provinceData.label }); // push province filter
      goToTab('uc2');
    } else {
      const provinceFilter = provinceData.label || '';
