...
import ToanQuocIms from "./ToanQuocIms";
import MienBac from "./MienBac";
import MienNam from "./MienNam";
import MienBacIms from "./MienBac_Ims";
import MienNamIms from "./MienNam_Ims";
    ...
    // <div style={{ width: '100%' }}>
        ...
        {tab === "dashboard" && (
          <span className="position-relative end-0 me-3 bg-info-subtle text-info-emphasis shadow-lg rounded-3 p-2">
            ...
            <select
              value={loaiHinh}
              onChange={(e) => {
                setLoaiHinh(e.target.value);
                setIsDisplayed(false);
              }}
              className="border rounded px-2 w-40 mb-2"
            >
              <option value="aaranet">Aaranet</option>
      ...
      <div style={{ padding: "1rem" }}>
        {tab === "dashboard" && (
          <>
            {/* Aaranet */}
            {combinedOption === "aaranet toanquoc" && (
              <ToanQuoc isDisplayed={isDisplayed} />
            )}
            {combinedOption === "aaranet mienbac" && (
              <MienBac isDisplayed={isDisplayed} />
            )}
            {combinedOption === "aaranet miennam" && (
              <MienNam isDisplayed={isDisplayed} />
            )}

            {/* IMS */}
            {combinedOption === "ims toanquoc" && (
              <ToanQuocIms isDisplayed={isDisplayed} />
            )}
            {combinedOption === "ims mienbac" && (
              <MienBacIms isDisplayed={isDisplayed} />
            )}
            {combinedOption === "ims miennam" && (
              <MienNamIms isDisplayed={isDisplayed} />
            )}
          </>
        )}
          {tab === "giamsat" && <Giamsat />}
