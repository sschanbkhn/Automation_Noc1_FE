...
import BoxThongKe from "./Box_thong_ke_MN_IMS";
import BangThongTinDoiSoatIms from "./Bang_thong_tin_doi_soat_IMS";
import Circle_box_ds_MN_Ims from "./Circle_box_ds_MN_Ims"
...
interface MienNamImsProps {
  isDisplayed: boolean;
}

function MienNamIms({ isDisplayed }: MienNamImsProps) {
  type ThietBi = {
    number: string;
    MTAS: string;
    pbx1: string;
  ...
  const displayDanhSachLechAccName = () => {
    ...
    fetch(`${API_URL}/thong-ke-loi/ims/mn/khong_dung_mtas/`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const displayDanhSachLechKenh = () => {
    setLoading(true);
    setDanhSachLechKenh([]);
    fetch(`${API_URL}/thong-ke-loi/ims/mn/khong_dung_kenh/`)
      .then((res) => res.json())
      .then((data) => {
        const list = Array.isArray(data) ? data : [];
  ...
  const displayDanhSachLechNumber = () => {
    ...
    fetch(`${API_URL}/thong-ke-loi/ims/mn/khong_dung_siptel/`)
      .then((res) => res.json())
      .then((data) => {
        const list = Array.isArray(data) ? data : [];
  ...
  const displayDanhSachLechDest = () => {
    setLoading(true);
    setDanhSachLechDest([]);
    fetch(`${API_URL}/thong-ke-loi/ims/mn/khong_dung_pbx1/`)
      .then((res) => res.json())
      .then((data) => {
        const list = Array.isArray(data) ? data : [];
  ...
  const displayDoiSoatKenh = () => {
    fetch(`${API_URL}/doi-soat/ims/mn/khong_dung_kenh/`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const displayDoiSoatAccName = () => {
    fetch(`${API_URL}/doi-soat/ims/mn/khong_dung_acc_name/`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const displayDoiSoatDest = () => {
    fetch(`${API_URL}/doi-soat/ims/mn/khong_dung_dest/`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const displayDoiSoatMoQte = () => {
    fetch(`${API_URL}/doi-soat/ims/mn/khong_dung_loaihinh_moqte/`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const displayDoiSoatKhoaQte = () => {
    fetch(`${API_URL}/doi-soat/ims/mn/khong_dung_loaihinh_khoaqte/`)
      .then((res) => res.json())
      .then((data) => {
      ...
      <div style={{ width: '100%' }}>
        ...
        <div style={{ marginTop: "10px", display: 'flex', gap: '20px' }}>
          ...
          <div className="box_phai_zone1" style={{ width: '50%', height: '65vh' }}>
            ...
            <Circle_box_ds_MN_Ims onClickBox={handleClickPieSlice_ds} isDisplayed={isDisplayed} />  {/* ✅ truyền callback */}
          </div>
        </div>
        {/* Khung zone 2*/}
