...
const COLORS = ['#79c400ff', '#bb28ffff','#FFBB28',
  '#fe0015ff','#0088FE', '#00C49F', '#ff28adff',
  '#00fe2aff', '#c4b000ff', , '#28ffadff'
];
...
export default function PieWithPercentage({ onZoneClick, isDisplayed }: Props) {
  ...
  const keys = ["spid", "name", "tps", "shortcode", "ips", "add", "url"];
  const labels = ["SP ID", "Name", "TPS", "ShortCode", "IP", "Address", "URL"];

  useEffect(() => {
    if (!isDisplayed) return;

    const endpoints = [
      "khong_dung_spid", "khong_dung_name", "khong_dung_tps", "khong_dung_shortcode",
      "khong_dung_ip","khong_dung_address", "khong_dung_url"
    ];
